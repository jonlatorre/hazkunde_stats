...
class resultados_pre_ideas_cantabria (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    def save(self, **kwargs):
    ...
    class Meta:
        #Nombre de la tabla
        db_table="resultados_pre_ideas_cantabria"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"
...
class resultados_pre_conocimientos_navarra (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    def save(self, **kwargs):
    ...
    class Meta:
        #Nombre de la tabla
        db_table="resultados_pre_conocimientos_navarra"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"

class resultados_pre_conocimientos_cantabria (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    def save(self, **kwargs):
    ...
    class Meta:
        #Nombre de la tabla
        db_table="resultados_pre_conocimientos_cantabria"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"
...
# CREATE VIEW resultados_post_ideas_cantabria AS select question_id - 46 as pregunta, answer as respuesta, count(*) as numero,CONCAT(question_id-1,"-",answer) AS id from hazkundedokeos_dokeos_stats.track_e_attempt where question_id-46 IN (1,2,3,4,5,6,7,8,9) AND hazkundedokeos_dokeos_stats.track_e_attempt.user_id in (select id_user FROM usuarios_cantabria) GROUP BY question_id, answer;
class resultados_post_ideas_cantabria (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    class Meta:
        #Nombre de la tabla
        db_table="resultados_post_ideas_cantabria"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"
...
#CREATE VIEW  resultados_post_conocimientos_navarra AS select question_id - 16 as pregunta, answer as respuesta, count(*) as numero,CONCAT(question_id-1,"-",answer) AS id from hazkundedokeos_dokeos_stats.track_e_attempt where question_id-16 IN (1,2,3,4,5,6,7,8,9) AND hazkundedokeos_dokeos_stats.track_e_attempt.user_id in (select id_user FROM usuarios_navarra) GROUP BY question_id, answer;
class resultados_post_conocimientos_navarra (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    class Meta:
        #Nombre de la tabla
        db_table="resultados_post_conocimientos_navarra"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"

#CREATE VIEW  resultados_post_conocimientos_cantabria AS select question_id - 16 as pregunta, answer as respuesta, count(*) as numero,CONCAT(question_id-1,"-",answer) AS id from hazkundedokeos_dokeos_stats.track_e_attempt where question_id-16 IN (1,2,3,4,5,6,7,8,9) AND hazkundedokeos_dokeos_stats.track_e_attempt.user_id in (select id_user FROM usuarios_cantabria) GROUP BY question_id, answer;
class resultados_post_conocimientos_cantabria (models.Model):
    pregunta = models.DecimalField(max_digits=6, decimal_places=0)
    respuesta = models.DecimalField(max_digits=6, decimal_places=0)
    numero = models.DecimalField(max_digits=6, decimal_places=0)
    class Meta:
        #Nombre de la tabla
        db_table="resultados_post_conocimientos_cantabria"
        #No la gestionamos, porque realmente es una vista
        managed="False"
        #Por si tenemos que enrutar las conexiones a BBDD
        app_label="stats_rt"
